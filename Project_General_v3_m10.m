...
SimParams.K = min(4,SimParams.Ktotal/4);
...
SimParams.L_v = 20*ones(1,SimParams.U);
...
SimParams.K_factor = [0,2,1,3];
SimParams.Heights = [15,45,20,15];
...
%         SimParams.u = u;
%         SimParams.dist(:,u,l) = norm(SimParams.Rx_pos(:,u,l) - SimParams.Tx_pos);
...
bRays = 20;
aRays = 4;
SimParams.R = round(rand(1,1)*(bRays - aRays) + aRays);
SimParams.R_reduced = (bRays + aRays)/2;
Rx_pos = zeros(3,SimParams.U,(SimParams.C-1)*SimParams.R + 1);
SimParams.L_v = (SimParams.C - 1)*SimParams.R + 1;
SimParams.L_v_reduced = (SimParams.C - 1)*SimParams.R_reduced + 1;
for u = 1:SimParams.U
    SimParams.Tx_pos = Tx_pos;
    for c = 1:SimParams.C
        if c == 1
            Rx_pos_aux = 30*randn(3,SimParams.U,1);
            Rx_pos_aux(3,:,:) = SimParams.Heights(u);
            SimParams.Rx_pos(:,u,1) = Rx_pos_aux;
        else
            Rx_pos_aux2 = 120*rand(3,SimParams.U,1) - 60;
            Rx_pos_aux = [];
            for r = 1:SimParams.R
                Rx_pos_aux(:,u,r) = Rx_pos_aux2 + 1e-1*randn(3,1,1);
            end
%             Rx_pos_aux(3,:,:) = abs(Rx_pos_aux(3,:,:));
            SimParams.Rx_pos(:,u,1 + (c-2)*SimParams.R + (1:SimParams.R)) = Rx_pos_aux;
        end 
    end
end
...
SimParams.Rx_pos_av = zeros(3,SimParams.U,(SimParams.C - 1)*SimParams.R_reduced + 1);
for u = 1:SimParams.U
    SimParams.Tx_pos = Tx_pos;
    for c = 1:SimParams.C
        if c == 1
            ...
            Rx_pos_aux2 = 120*rand(3,SimParams.U,1) - 60;
            Rx_pos_aux = [];
            for r = 1:SimParams.R_reduced
                Rx_pos_aux(:,u,r) = Rx_pos_aux2 + 1e-1*randn(3,1,1);
            end
%             Rx_pos_aux(3,:,:) = abs(Rx_pos_aux(3,:,:));
            SimParams.Rx_pos_av(:,u,1 + (c-2)*SimParams.R_reduced + (1:SimParams.R_reduced)) = Rx_pos_aux;
        end 
    end
end
...
Sync_training_time = 0;%SimParams.Mtck*(SimParams.Zp + SimParams.Ntr)*SimParams.Ts;
Sync_data_time = 0;%(SimParams.Zp + SimParams.Ntr)*SimParams.Ts;
Data_time = SimParams.Bdata*(SimParams.Zp + SimParams.Ktotal)*SimParams.Ts;
SimParams.Ttck = Sync_training_time + Training_time + Sync_data_time + Data_time + 100e-6;
...
% Correlation coefficients
for u = 1:SimParams.U
    for l = 1:SimParams.L_v_reduced(1)
%         SimParams.rho(u,l) = cos(0.45*dx(u)/SimParams.lambda_c)*exp(-0.1*dx(u)/SimParams.lambda_c);
        factor_LOS = SimParams.K_factor_nat(u)/(SimParams.K_factor_nat(u)+1);
        factor_NLOS = 1/(SimParams.K_factor_nat(u)+1);
        SimParams.rho_reduced(u,l) = 0.99*exp(-dx(u)*2.05)*(l == 1) + (0.9*exp(-dx(u)*1.05) - 0.1)*(l > 1);
%         SimParams.rho(u,l) = sqrt(SimParams.Nbs*SimParams.Nms_v(1)/SimParams.L_v(1))*SimParams.rho(u,l);
%     SimParams.rho_NLOS(u) = exp(-0.26*dx(u)/SimParams.lambda_c);
%     fd = SimParams.f_c*norm(diff_v(:,u))/SimParams.c;
%     Tblock = SimParams.Ttck;
...
SimParams.rho_reshaped_reduced = diag(reshape(SimParams.rho_reduced.',[],1));
SimParams.Rho_reduced = zeros(SimParams.U*SimParams.K*SimParams.L_v_reduced(1),SimParams.U*SimParams.K*SimParams.L_v_reduced(1));

for k = 1:SimParams.K
    for j = 1:SimParams.K
        SimParams.Rho_reduced((k-1)*SimParams.L_v_reduced(1)*SimParams.U + (1:SimParams.U*SimParams.L_v_reduced(1)), (j-1)*SimParams.L_v_reduced(1)*SimParams.U + (1:SimParams.U*SimParams.L_v_reduced(1))) = exp(-abs((k-j)*SimParams.B/SimParams.K*SimParams.Ttck))*SimParams.rho_reshaped_reduced;
    end
end
...
a_p = -40;
b_p = 40;
tau = sort(rand(SimParams.U,SimParams.L_v(1),1))*(SimParams.Zp - 1)*SimParams.Ts;
Mfilter = 1;
q = 1;
%     q = 0.01;
AS_azi = 1*(pi/180);
...
theta_AoA_cluster = rand(1,SimParams.C)*pi;
theta_AoD_cluster = rand(1,SimParams.C)*pi;
phi_AoA_cluster = rand(1,SimParams.C)*pi/2;
phi_AoD_cluster = rand(1,SimParams.C)*pi/2;

for u = 1:SimParams.U
    std_angle(u) = sqrt(q*norm(diff_v(:,u),2)^2*SimParams.Ttck/100^2);
...
%         SimParams.phi_AoD_ini(u,l) = rand(1,1)*pi/2;%phi_1u2;
        ...
        if l == 1
            SimParams.theta_AoA_ini(u,l) = theta_AoA_cluster(1);
            SimParams.theta_AoD_ini(u,l) = theta_AoD_cluster(1);
            SimParams.phi_AoA_ini(u,l) = phi_AoA_cluster(l);
            SimParams.phi_AoD_ini(u,l) = phi_AoD_cluster(l);
        else
            for r = 1:SimParams.R
                SimParams.theta_AoA_ini(u,1 + (l - 2)*SimParams.R + (1:SimParams.R)) = theta_AoA_cluster(l) + std_angle(u)/sqrt(2)*randl(1,SimParams.R,1);
                SimParams.theta_AoD_ini(u,1 + (l - 2)*SimParams.R + (1:SimParams.R)) = theta_AoD_cluster(l) + std_angle(u)/sqrt(2)*randl(1,SimParams.R,1);
                SimParams.phi_AoA_ini(u,1 + (l - 2)*SimParams.R + (1:SimParams.R)) = phi_AoA_cluster(l) + std_angle(u)/sqrt(2)*randl(1,SimParams.R,1);
                SimParams.phi_AoD_ini(u,1 + (l - 2)*SimParams.R + (1:SimParams.R)) = phi_AoD_cluster(l) + std_angle(u)/sqrt(2)*randl(1,SimParams.R,1);
            end
        end
        
        for l = 1:SimParams.L_v(1)
            SimParams.d_1u(u,l) = rand(1,1)*60 - 30;
            SimParams.d_1u2(u,l) = rand(1,1)*60 - 30;
    ...
    for l = 1:SimParams.L_v(u)
        ...
        SimParams.tau(u,l,1) = tau(u,l,1);%abs(SimParams.d1(u,l,1) - SimParams.d2(u,l,1))/3e8;
    end
end
...
for u = 1:SimParams.U
    for l = 1:SimParams.C%SimParams.L_v_reduced(u)
        SimParams.u = u;
        Rel_pos(:,u,l) = SimParams.Tx_pos - SimParams.Rx_pos(:,u,l);% (b_p - a_p)*rand(3,1) + a_p;
        Rel_pos2(:,u,l) = (SimParams.Rx_pos(:,u,1) - SimParams.Tx_pos)*(l == 1) + (SimParams.Rx_pos(:,u,1) - SimParams.Rx_pos(:,u,l))*(l > 1);
        [d_1u,theta_1u,phi_1u] = TransfC2S(Rel_pos(1,u,l),Rel_pos(2,u,l),Rel_pos(3,u,l));
...
%         SimParams.phi_AoD_ini(u,l) = rand(1,1)*pi/2;%phi_1u2;
        if l == 1
            SimParams.theta_AoA_ini_reduced(u,l) = theta_AoA_cluster(l);% + std_angle(u)/sqrt(2)*randl(1,1,1);
            SimParams.theta_AoD_ini_reduced(u,l) = theta_AoD_cluster(l);% + std_angle(u)/sqrt(2)*randl(1,1,1);
            SimParams.phi_AoA_ini_reduced(u,l) = phi_AoA_cluster(l);% + std_angle(u)/sqrt(2)*randl(1,1,1);
            SimParams.phi_AoD_ini_reduced(u,l) = phi_AoD_cluster(l);% + std_angle(u)/sqrt(2)*randl(1,1,1);
        else
        for r = 1:SimParams.R_reduced
            SimParams.theta_AoA_ini_reduced(u,1 + (l - 2)*SimParams.R_reduced + (1:SimParams.R_reduced)) = theta_AoA_cluster(l) + std_angle(u)/sqrt(2)*randl(1,SimParams.R_reduced,1);
            SimParams.theta_AoD_ini_reduced(u,1 + (l - 2)*SimParams.R_reduced + (1:SimParams.R_reduced)) = theta_AoD_cluster(l) + std_angle(u)/sqrt(2)*randl(1,SimParams.R_reduced,1);
            SimParams.phi_AoA_ini_reduced(u,1 + (l - 2)*SimParams.R_reduced + (1:SimParams.R_reduced)) = phi_AoA_cluster(l) + std_angle(u)/sqrt(2)*randl(1,SimParams.R_reduced,1);
            SimParams.phi_AoD_ini_reduced(u,1 + (l - 2)*SimParams.R_reduced + (1:SimParams.R_reduced)) = phi_AoD_cluster(l) + std_angle(u)/sqrt(2)*randl(1,SimParams.R_reduced,1);
        end
        end
...
for u = 1:SimParams.U
    for l = 1:SimParams.L_v(u)
        ...
        factor_LOS = SimParams.K_factor_nat(u)/(SimParams.K_factor_nat(u)+1);
        factor_NLOS = 1/(SimParams.K_factor_nat(u)+1);
        SimParams.Q_Gains(:,:,u,l) = eye(SimParams.K)*(1 - SimParams.rho(u,l)*1/SimParams.K_factor_nat(u)*SimParams.rho(u,l));
        SimParams.Q_Gains(:,:,u,l) = SimParams.Q_Gains(:,:,u,l);
    end
end
...
for u = 1:SimParams.U
    for l = 1:SimParams.L_v_reduced(u)
        F1 = F(:,1:SimParams.Zp);
        SimParams.Pbar_0_Gains_reduced(1:SimParams.K,1:SimParams.K,u,l) = F1*F1';
        SimParams.Q_Gains_reduced(:,:,u,l) = eye(SimParams.K)*(1 - SimParams.rho_reduced(u,l)*1/SimParams.K_factor_nat(u)*SimParams.rho_reduced(u,l));
    end
end
...
for nmc = 1:NMC
    ...
    SimParams.m_gains_0_vect_reduced = zeros(SimParams.K*SimParams.U*SimParams.L_v_reduced,1);
    Gt = [];
    for u = 1:SimParams.U
        ...
        % AVERAGE IMPLEMENTATION
        gains_u_reduced = zeros(SimParams.Zp,SimParams.L_v_reduced);
        gains_u_fd_reduced(:,:,u) = F(:,1:SimParams.Zp)*gains_u_reduced;
        
    end
    for k = 1:SimParams.K
        SimParams.m_gains_0_vect((k-1)*SimParams.L_v(1)*SimParams.U + (1:SimParams.L_v(1)*SimParams.U)) = reshape(gains_u_fd(k,:,:),[],1);
        SimParams.m_gains_0_vect_reduced((k-1)*SimParams.L_v_reduced(1)*SimParams.U + (1:SimParams.L_v_reduced(1)*SimParams.U)) = reshape(gains_u_fd_reduced(k,:,:),[],1);
    end
...
%     SimParams.m_gains_0_vect = 1/sqrt(2)*(randn(SimParams.K*SimParams.U*SimParams.L_v(1),1) + 1i*randn(SimParams.K*SimParams.U*SimParams.L_v(1),1));%ones(SimParams.K,1);
    ...
    for k = 1:SimParams.K
        for j = 1:SimParams.K
            Pbar_current = squeeze(SimParams.Pbar_0_Gains_reduced(k,j,:,:));
            Pbar_Gains_reduced((k-1)*SimParams.U*SimParams.L_v_reduced(1) + (1:SimParams.U*SimParams.L_v_reduced(1)),(j-1)*SimParams.U*SimParams.L_v_reduced(1)+(1:SimParams.U*SimParams.L_v_reduced(1))) = diag(reshape(Pbar_current.',[],1));
%             Pbar_current2 = squeeze(SimParams.Pbar_0_Gains2(k,j,:,:));
%             Pbar_Gains2_reduced((k-1)*SimParams.U*SimParams.L_v_reduced(1) + (1:SimParams.U*SimParams.L_v_reduced(1)),(j-1)*SimParams.U*SimParams.L_v_reduced(1)+(1:SimParams.U*SimParams.L_v_reduced(1))) = diag(reshape(Pbar_current2.',[],1));
            Q_current = squeeze(SimParams.Q_Gains_reduced(k,j,:,:));
            Q_Gains_reduced((k-1)*SimParams.U*SimParams.L_v_reduced(1) + (1:SimParams.U*SimParams.L_v_reduced(1)),(j-1)*SimParams.U*SimParams.L_v_reduced(1)+(1:SimParams.U*SimParams.L_v_reduced(1))) = diag(reshape(Q_current.',[],1));
        end
    end
    ...
    SimParams.Pbar_0_Gains_reduced = Pbar_Gains_reduced;
    SimParams.Q_Gains_reduced = Q_Gains_reduced;
...
%     SimParams.m_gains_0_vect = sqrt(1/2)*(randn(SimParams.K*SimParams.U*SimParams.L_v(1),1) + 1i*randn(SimParams.K*SimParams.U*SimParams.L_v(1),1));
...
%     SimParams.Gains_vect_reduced(:,1) = SimParams.m_gains_0_vect_reduced + sqrtm(SimParams.Pbar_0_Gains_reduced/2)*(randn(SimParams.K*SimParams.U*SimParams.L_v(1),1) + 1i*randn(SimParams.K*SimParams.U*SimParams.L_v(1),1));
%     SimParams.Gains_vect_reduced(:,1) = SimParams.Rho_reduced*SimParams.Gains_vect_reduced(:,1) + sqrtm(SimParams.Q_Gains_reduced/2)*(randn(SimParams.K*SimParams.U*SimParams.L_v_reduced(1),1) + 1i*randn(SimParams.K*SimParams.U*SimParams.L_v(1),1));
    ...
    for t = 2:length(SimParams.Time)
        ...
        for u = 1:SimParams.U
            for l = 1:SimParams.L_v(1)
                ...
                SimParams.tau(u,l,t) = abs(SimParams.d1(u,l,t) - SimParams.d2(u,l,t))/3e8;
            end
        end
...
%         SimParams.Channel(:,t) = kron(eye(SimParams.K),SimParams.Psi)*SimParams.Gains_vect(:,t);
    ...
    for t = 1:length(SimParams.Time)
        ...
        for u = 1:SimParams.U
                SimParams.u = u;
            [Hk,norm_factor] = gen_channel_UPA(SimParams,SimParams.theta_AoA(:,:,t),SimParams.theta_AoD(:,:,t),SimParams.phi_AoA(:,:,t),SimParams.phi_AoD(:,:,t),Gains_vect_t);
            %         SimParams.Hk_MU{u,1} = Hk;
