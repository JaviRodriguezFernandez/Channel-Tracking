...
for snri = 1:length(SNR_v)
    Percentage_snr = snri/length(SNR_v)*100
    for miter = 3:length(Mtck)
        Percentage_Mtck = miter/length(Mtck)*100
        for nsi = 1:length(Ns)
            Percentage_Ns = nsi/length(Ns)*100
            rng(0);

SimParams = struct;
SNR = SNR_v(snri);
snr = snr_v(snri);
SimParams.SNR_current = SNR;
SimParams.Mtck_current = Mtck(miter);
SimParams.Ns_current = Ns(nsi);
...
SimParams.K = min(16,SimParams.Ktotal/4);
...
for u = 1:SimParams.U
    ...
    for c = 1:SimParams.C
        if c == 1
            ...
            Rx_pos_aux = [];
            for r = 1:SimParams.R
                Rx_pos_aux(:,u,r) = Rx_pos_aux2 + 1e-1*rand(3,1,1);
            end
...
Rx_pos_true_aux = reshape(SimParams.Rx_pos(:,1,2:end),[3,SimParams.R,SimParams.C-1]);
for u = 1:SimParams.U
    SimParams.Tx_pos = Tx_pos;
    for c = 1:SimParams.C
        if c == 1
            Rx_pos_aux = SimParams.Rx_pos(:,u,c) + 0.1*rand(3,1,1);
%             Rx_pos_aux(3,:,:) = SimParams.Heights(u);
            SimParams.Rx_pos(:,u,1) = Rx_pos_aux;
        else
            for r = 1:SimParams.R_reduced
                Rx_pos_aux(:,u,r) = Rx_pos_true_aux(:,randi(SimParams.R,1,1),c-1) + 0.1*rand(3,1,1);
            end
%             Rx_pos_aux(3,:,:) = abs(Rx_pos_aux(3,:,:));
...
%         SimParams.phi_AoA_ini(u,l) = rand(1,1)*pi/2;%phi_1u;
%         SimParams.phi_AoD_ini(u,l) = rand(1,1)*pi/2;%phi_1u2;
            SimParams.theta_AoA_ini_reduced(u,l) = theta_1u + std_angle(u)/sqrt(2)*randl(1,1,1);
            SimParams.theta_AoD_ini_reduced(u,l) = theta_1u2 + std_angle(u)/sqrt(2)*randl(1,1,1);
            SimParams.phi_AoA_ini_reduced(u,l) = phi_1u + std_angle(u)/sqrt(2)*randl(1,1,1);
            SimParams.phi_AoD_ini_reduced(u,l) = phi_1u2 + std_angle(u)/sqrt(2)*randl(1,1,1);
