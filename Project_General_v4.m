...
SimParams = struct;
% load QuaDRiGa_KF_0_dB_20m_dist_300_speed.mat;
L = 20;
% SimParams.Gains_orig = sqrt(Power(1,1:L)).*exp(1i*Phase(1,1:size(Power(1,1:L))));
% SimParams.ToA = ToA(1,1:L);
% SimParams.theta_AoA_ini = mod(AoA(1,1:L),pi);
% SimParams.theta_AoD_ini = mod(AoD(1,1:L),pi)
% SimParams.phi_AoA_ini = pi/2 - AoA(1,1:L);
% SimParams.phi_AoD_ini = pi/2 - AoD(1,1:L);
load Channel_Parameters_KF_0_dB_20m_300speed.mat;
SimParams.phi_AoA_ini = rand(1,L)*pi - pi/2;
SimParams.phi_AoD_ini = rand(1,L)*pi - pi/2;
d_k = [20,rand(1,length(SimParams.Gains_orig))*100];
d_k2 = [20,rand(1,length(SimParams.Gains_orig))*100];
SimParams.U = 1;
...
SimParams.L_v = length(SimParams.Gains_orig)*ones(1,SimParams.U);
...
for u = 1:SimParams.U
    for l = 1:SimParams.L_v(u)
        SimParams.u = u;
        SimParams.d1(u,l,1) = d_k(1);
        SimParams.d2(u,l,1) = d_k2(1);
...
%         %             SimParams.phi_AoD_ini(u,l) = phi_1u2;
        SimParams.d_1u(u,l) = SimParams.d1(u,l,1);
        SimParams.d_1u2(u,l) = SimParams.d2(u,l,1);
        SimParams.tau_1u(u,l) = SimParams.ToA(1,l);
        ...
        std_angle(u) = sqrt(q*norm(diff_v(:,u),2)^2*SimParams.Ttck/SimParams.d_1u(u,1)^2);
        ...
        SimParams.tau(u,l,1) = SimParams.ToA(1,l);
    end
end
...
for u = 1:SimParams.U
    for l = 1:SimParams.L_v(u)
        ...
        SimParams.Q_Gains(:,:,u,l) = SimParams.Q_Gains(:,:,u,l)*(l == 1) + SimParams.Q_Gains(:,:,u,l)*(l > 1);
    end
end
    ...
    for t = 2:length(SimParams.Time)
        ...
        for u = 1:SimParams.U
            for l = 1:SimParams.L_v(1)
                ...
                SimParams.tau(u,l,t) = SimParams.tau(u,l,1);
            end
        end
...
%         SimParams.Channel(:,t) = kron(eye(SimParams.K),SimParams.Psi)*SimParams.Gains_vect(:,t);
    ...
    for t = 1:length(SimParams.Time)
        ...
        for u = 1:SimParams.U
                SimParams.u = u;
                if t == 1
                    [Hk,norm_factor] = gen_channel_ULA(SimParams,SimParams.theta_AoA(:,:,t),SimParams.theta_AoD(:,:,t),Gains_vect_t);
                else
...
%     SNR
    ...
    SimParams.N = 1000;
    SimParams.nmc = nmc;
    SimParams = RBPF_Project_General_v4_SWOMP(SimParams);
