...
SimParams.L_v = 6*ones(1,SimParams.U);
...
%             SimParams.phi_AoD_ini(u,l) = phi_1u2;
    ...
    for u = 1:SimParams.U
        for l = 1:SimParams.L_v(u)            
            ...
            SimParams.Q_Gains(:,:,u,l) = eye(SimParams.K)*(1 - SimParams.rho(u,l)*SimParams.rho(u,l));%SimParams.Nbs*SimParams.Nms_v(1)/SimParams.L_v(1)*(1 - SimParams.rho(u,l)*SimParams.rho(u,l));
        end
    end
...
%     end
    ...
    for t = 2:length(SimParams.Time)
        ...
        SimParams.Q_Gains = Q_Gains;%*Q_Gains_ext*Q_Gains';
...
%         SimParams.Channel(:,t) = kron(eye(SimParams.K),SimParams.Psi)*SimParams.Gains_vect(:,t);
    ...
    for t = 1:length(SimParams.Time)
        ...
        for u = 1:SimParams.U
                SimParams.u = u;
            Hk = gen_channel_UPA(SimParams,SimParams.theta_AoA(:,:,t),SimParams.theta_AoD(:,:,t),SimParams.phi_AoA(:,:,t),SimParams.phi_AoD(:,:,t),Gains_vect_t);
            %         SimParams.Hk_MU{u,1} = Hk;
