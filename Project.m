...
SimParams.B = (1+SimParams.rolloff)*SimParams.fs;
...
for u = 1:SimParams.U
    ...
    dx(u) = SimParams.Ttck*norm(diff_v(:,u),2);
end
...
%     q = 0.01;
    ...
    for u = 1:SimParams.U
            ...
            SimParams.theta_AoA(u,1) = theta_1u + AS_azi/sqrt(2)*randl(1,1,1);
            SimParams.phi_AoA(u,1) = phi_1u + AS_ele/sqrt(2)*randl(1,1,1);
            ...
            SimParams.theta_AoA(u,1) = theta_k + std_angle(u)/sqrt(2)*randl(1,1,1);
            SimParams.phi_AoA(u,1) = phi_k + std_angle(u)/sqrt(2)*randl(1,1,1);
            SimParams.phi_AoD(u,1) = pi/2 - SimParams.phi_AoA(u,1);
            SimParams.theta_AoD(u,1) = pi - SimParams.theta_AoA(u,1);
            SimParams.d(u,1) = d_k;
            SimParams.tau(u) = d_k/3e8;
            
            for d = 0:SimParams.Zp-1
                Gt(d+1,u) = sinc((d*SimParams.Ts-SimParams.tau(u))/Mfilter/SimParams.Ts)*cos(pi*SimParams.rolloff*(d*SimParams.Ts-SimParams.tau(u))/Mfilter/SimParams.Ts)/(1-(2*SimParams.rolloff*(d*SimParams.Ts-SimParams.tau(u))/Mfilter/SimParams.Ts)^2);
            end
            F1 = F(:,1:SimParams.Zp)*diag(Gt(:,u));
            SimParams.Pbar_0_Gains(1:SimParams.K,1:SimParams.K,u) = F1*F1'*eye(SimParams.K,SimParams.K)*SimParams.Nbs*SimParams.Nms_v(u);
            SimParams.Pbar_0_Gains2(1:SimParams.K,1:SimParams.K,u) = F(:,1:SimParams.Zp)*F(:,1:SimParams.Zp)'*SimParams.Nbs*SimParams.Nms_v(u);
    ...
    for u = 1:SimParams.U
        SimParams.Pbar_Gains = blkdiag(SimParams.Pbar_Gains,SimParams.Pbar_0_Gains(:,:,u));
        SimParams.Pbar_Gains2 = blkdiag(SimParams.Pbar_Gains2,SimParams.Pbar_0_Gains2(:,:,u));
    ...
    for t = 2:length(SimParams.Time)
        for u = 1:SimParams.U
            ...
            SimParams.d(u,t) = d_k;
            SimParams.theta_AoA(u,t) = theta_k + std_angle(u)/sqrt(2)*randl(1,1,1);%std_angle(u)*randn(1,1);
            SimParams.phi_AoA(u,t) = phi_k + std_angle(u)/sqrt(2)*randl(1,1,1);%std_angle(u)*randn(1,1);
            ...
            F1 = F(:,1:SimParams.Zp)*diag(Gt(:,u));
            SimParams.Q_Gains_u(1:SimParams.K,1:SimParams.K,u) = F1*F1';%*sqrt(SimParams.Nbs*SimParams.Nms_v(u));
            SimParams.Q_Gains_u2(1:SimParams.K,1:SimParams.K,u) = F(:,1:SimParams.Zp)*F(:,1:SimParams.Zp)';%*sqrt(SimParams.Nbs*SimParams.Nms_v(u));
        end
        SimParams.Q_Gains_w = [];
        SimParams.Q_Gains_w2 = [];
        for u = 1:SimParams.U
            SimParams.Q_Gains_w = blkdiag(SimParams.Q_Gains_w,SimParams.Q_Gains_u(:,:,u));
            SimParams.Q_Gains_w2 = blkdiag(SimParams.Q_Gains_w2,SimParams.Q_Gains_u2(:,:,u));
        end
        
        SimParams.Q_Gains_final = SimParams.Q_Gains*SimParams.Q_Gains_w*SimParams.Q_Gains';
        
        SimParams.Gains_vect(:,t) = SimParams.Rho*SimParams.Gains_vect(:,t-1) + sqrtm(SimParams.Q_Gains_final/2)*(randn(SimParams.K*SimParams.U,1)+1i*randn(SimParams.K*SimParams.U,1));
    ...
    SimParams.Q_Gains = SimParams.Q_Gains*SimParams.Q_Gains_w2*SimParams.Q_Gains';
...
%     SimParams.Channel = sqrt(Scaling_factor)*SimParams.Channel;
    ...
    for t = 1:length(SimParams.Time)
        ...
        SNR(t,nmc) = Signal_energy(t)/Noise_energy(t);
    end
