...
SimParams.L_v = 2*ones(1,SimParams.U);
...
%     q = 0.01;
    ...
    for u = 1:SimParams.U
        for l = 1:SimParams.L_v(u)
            ...
            SimParams.Pbar_0_Gains(1:SimParams.K,1:SimParams.K,u,l) = eye(SimParams.K);%F1*Scaling_factor*F1';%*eye(SimParams.K,SimParams.K)*SimParams.Nbs*SimParams.Nms_v(u);
...
%     SimParams.Q_Gains = SimParams.Q_Gains*kron(eye(SimParams.K*SimParams.L_v(1)),diag(SimParams.Nbs*SimParams.Nms_v));%/SimParams.L_v(1));
    ...
    SimParams.m_gains_0_vect = 1/sqrt(2)*(randn(SimParams.K,1) + 1i*randn(SimParams.K,1));%ones(SimParams.K,1);
...
%     SNR
    ...
    SimParams = RBPF_Project_General_v2(SimParams);
